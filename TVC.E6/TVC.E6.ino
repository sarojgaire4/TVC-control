...
void loop() {
  ...
  int servoPitch = abs(compPitch) < gimbalDeadzone ? center : map(compPitch, -tiltRange, tiltRange, center - radius, center + radius);
  int servoRoll = abs(compRoll) < gimbalDeadzone ? center : map(compRoll, -tiltRange, tiltRange, center - radius, center + radius);
  int servoRollControl = gyroZrate == 0 ? center :
                         map(constrain(gyroZrate, -100, 100), -100, 100, center - rollAmplitude, center + rollAmplitude);
